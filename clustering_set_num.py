...
def extracting(img_paths, clustered_info, cluster_num, frames_per_video):
    extract_num = frames_per_video//cluster_num
    ...
    for i in range(cluster_num):
        ...
        if len(component_list) < extract_num:
            return "Error"
        else:
            cluster_extracted_img_path += component_list

    return sorted(cluster_extracted_img_path)

def clustering(img_paths, cluster_num, components, frames_per_video):
    imgs = []
    ...
    model.fit(pca)#(reducted_imgs)
    print("4. k-means clear")
    clustered_info = model.labels_
    print(clustered_info)
    # 여기서 cluster 개수 check, cluster 적게 되면 pass
    if len(set(clustered_info)) < cluster_num:
        return "Error"
    extracted_img_paths = extracting(img_paths, clustered_info, cluster_num, frames_per_video)
    if extracted_img_paths == "Error":
        print("5. extract error")
        return "Error"
    else:
...
for class_path in tqdm(trimmed_classes):
    ...
    for vid_name_path in vid_name_paths:
        ...
        if img_num > frames_per_video:
            extracted_img_paths = clustering(img_paths, cluster_num=cluster_num, components=img_num//2, frames_per_video=frames_per_video)
            if extracted_img_paths == "Error":
