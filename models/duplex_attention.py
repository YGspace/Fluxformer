...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, thw_shape, Value, Flow=False):
        B, N, _ = x.shape
        if self.pool_first:
...
class MultiScaleBlock(nn.Module):
    def __init__(
        ...
        ###flow attention
        self.flow_attn = MultiScaleAttention(
            dim,
            att_dim,
            num_heads=num_heads,
            input_size=input_size,
    ...
    def forward(self, x, flow=None, thw_shape=None):
        x_norm = self.norm1(x)

        x_block, thw_shape_new, attn_v = self.attn(x_norm, thw_shape, 0)
        ...
        ##########flow          여기서 플로우 나오게 하고
        flow_norm = self.norm1(flow)

        flow_block, thw_shape_new, dummy = self.flow_attn(flow_norm, thw_shape, attn_v, Flow=True)
        if self.dim_mul_in_att and self.dim != self.dim_out:
            flow = self.proj(flow_norm)
        flow_res, _ = attention_pool(
            flow, self.pool_skip, thw_shape, has_cls_embed=self.has_cls_embed
        )
        if self.gamma_1 is not None:
            flow = flow_res + self.drop_path(self.gamma_1 * flow_block)
        else:
            flow = flow_res + self.drop_path(flow_block)
        flow_norm = self.norm2(flow)
        flow_mlp = self.mlp(flow_norm)
        if not self.dim_mul_in_att and self.dim != self.dim_out:
            flow = self.proj(flow_norm)
        if self.gamma_2 is not None:
            flow = flow + self.drop_path(self.gamma_2 * flow_mlp)
        else:
            flow = flow + self.drop_path(flow_mlp)

        #####flow############  여기서 크로스어텐션

        if self.gamma_2 is not None:
            x = x + self.drop_path(self.gamma_2 * x_mlp) + self.drop_path(self.gamma_2 * flow_mlp)
        else:
            x = x + self.drop_path(x_mlp) + self.drop_path(flow_mlp)

        if thw_shape:
            return x, flow, thw_shape_new
        else:
            return x
