...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., name="blockname"):
        super().__init__()
...
class MultiheadAttentionWithPreNorm(nn.Module):
    ...
    def __init__(self,
        ...
        #								  **kwargs)
        self.name = name
        self.attn = Attention(embed_dims, num_heads, qkv_bias=True, attn_drop=attn_drop, name=self.name)  # batch first
        self.proj_drop = nn.Dropout(proj_drop)
        dropout_p = layer_drop.pop('dropout_p')
        layer_drop = layer_drop.pop('type')
        self.layer_drop = layer_drop(dropout_p) if layer_drop else nn.Identity()
