...
def make_dataloader(name):
    ...
    if dataset_name == "HMDB51":
        ...
        clip_duration = num_frames_to_sample * sample_rate / fps
        print(dataset_name)
        data_module = ssv2DataModule(clip_duration)
        train_dataloader = data_module.train_dataloader()
    val_dataloader = data_module.val_dataloader()
    return train_dataloader,val_dataloader,num_classes,data_module.get_max_iter(),label2id


class ssv2DataModule(pytorch_lightning.LightningDataModule):
    # Training dataset transformations.
    def __init__(self,clip_duration):
        self.dataset_root_path = '/home/hong/workspace/datasets/ssv2_lite2/'
    ...
    def val_dataloader(self):
        ...
        val_dataset = pytorchvideo.data.Ucf101(
            ...
            clip_sampler=pytorchvideo.data.make_clip_sampler("constant_clips_per_video", self.clip_duration,5),
            decode_audio=False,
...
class kinetics400DataModule(pytorch_lightning.LightningDataModule):
    # Training dataset transformations.
    def __init__(self,clip_duration):
        self.dataset_root_path = '/home/hong/workspace/datasets/kinetics400_5per/data_cvt_split/'
        self.dataset_root_path = pathlib.Path(self.dataset_root_path)
        self.clip_duration = clip_duration
        video_count_train = len(list(self.dataset_root_path.glob("train/*/*.mp4")))
        video_count_val = len(list(self.dataset_root_path.glob("val/*/*.mp4")))
    ...
    def val_dataloader(self):
        ...
        val_dataset = pytorchvideo.data.Kinetics(
            data_path=os.path.join(self.dataset_root_path, "val"),
            clip_sampler=pytorchvideo.data.make_clip_sampler("constant_clips_per_video", self.clip_duration,5),#"constant_clips_per_video", self.clip_duration,5),
            decode_audio=False,
            transform=val_transform,
        )
        return torch.utils.data.DataLoader(
...
class kinetics600DataModule(pytorch_lightning.LightningDataModule):
    # Training dataset transformations.
    def __init__(self,clip_duration):
        self.dataset_root_path = '/home/hong/workspace/datasets/kinetics600_5per/data_cvt_split/'
        self.dataset_root_path = pathlib.Path(self.dataset_root_path)
        video_count_train = len(list(self.dataset_root_path.glob("train/*/*.mp4")))
        video_count_val = len(list(self.dataset_root_path.glob("val/*/*.mp4")))
        video_total = video_count_train + video_count_val
...
class HMDB51DataModule(pytorch_lightning.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        val_dataset = pytorchvideo.data.Hmdb51(
            data_path=self.dataset_root_path,
            clip_sampler=pytorchvideo.data.make_clip_sampler("constant_clips_per_video", self.clip_duration,5),#"uniform", self.clip_duration),
            decode_audio=False,
            transform=val_transform,
            split_id=self.split_id,
            split_type="test",
...
class SSV2DataModule(pytorch_lightning.LightningDataModule):
    ...
    def val_dataloader(self):
        ...
        val_dataset = pytorchvideo.data.SSv2(
            ...
            video_path_prefix="/home/hong/workspace/datasets/ssv2/",
            clip_sampler=pytorchvideo.data.make_clip_sampler("constant_clips_per_video", self.clip_duration,1),#"uniform", self.clip_duration),
            transform=val_transform,
        )
        return torch.utils.data.DataLoader(
            val_dataset,
