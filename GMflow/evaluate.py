...
def inference_on_dir(model,
    ...
    vid_classes = sorted(glob(inference_dir+"/*/"))

    for class_path in tqdm(vid_classes):
        c_names = sorted(glob(class_path+"*/"))
        only_class = class_path[len(inference_dir) + 1:-1]
        f_class_dir = flow_root + only_class
        i_class_dir = inference_dir + only_class
        if not check_dir(f_class_dir):
            os.mkdir(f_class_dir)

        for i_name in c_names:
            f_path = sorted(glob(i_name + "*"))
            only_name = i_name[len(class_path):-1]
            f_img_dir = flow_root + only_class + "/" + only_name
            i_img_dir = inference_dir + only_class + "/" + only_name

            if not check_dir(f_img_dir):
                os.mkdir(f_img_dir)

            filenames = sorted(glob(f_img_dir + '/*'))
